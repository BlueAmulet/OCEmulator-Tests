...
test.evaluate(unicode.char(0) == "\0")
test.evaluate(unicode.char(0/0) == "\0")
test.evaluate(unicode.char(math.huge) == "\0")
test.evaluate(unicode.char(-math.huge) == "\0")
...
test.evaluate(unicode.charWidth("\0") == 1)
...
test.evaluate(unicode.isWide("\0") == false)
...
test.evaluate(unicode.len("RATED\0X") == 7)
...
test.evaluate(unicode.wlen("RATED\0X") == 7)
...
test.evaluate(unicode.wtrunc("ABCDEFG", -math.huge) == "")
test.evaluate(unicode.wtrunc("ABCD\0EFG", 6) == "ABCD\0")
...
test.evaluate(unicode.lower("ABCΓΓΓ\0abcγγγ") == "abcγγγ\0abcγγγ")
test.evaluate(unicode.upper("ABCΓΓΓabcγγγ") == "ABCΓΓΓABCΓΓΓ")
test.evaluate(unicode.upper("abcγγγ\0abcγγγ") == "ABCΓΓΓ\0ABCΓΓΓ")
test.evaluate(unicode.reverse("Ümläüt") == "tüälmÜ")
test.evaluate(unicode.reverse("Üml\0äüt") == "tüä\0lmÜ")
...
test.evaluate(unicode.sub("Üml\0äüt",1) == "Üml\0äüt")
